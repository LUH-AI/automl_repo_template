...
def read_log(paths: str, loading_functions: List[Callable], processing_functions: List[Callable], outpath: Path, batch_size: int = 1000, n_processes: int = 4) -> pd.DataFrame:
    ...
    for i, batch in enumerate(batched(filenames, batch_size)):
        ...
        df = map_multiprocessing(task_functions=loading_functions, task_params=batch, task_string="Reading logs...", n_processes=n_processes)
...
def map_multiprocessing(task_functions: List[Callable], task_params: list[Any], n_processes: int = 4, task_string: str = "Working...") -> list:
    results = []

    def get_results(path):
        results = map(task_functions, path)
...
def read_logs(data_path: str, loading_functions: List[Callable], processing_functions: List[Callable], save_to: str) -> pd.DataFrame:
    path = Path(data_path)
    filenames = list(path.glob(f"**/*"))
    outpath = Path(save_to)
    return read_logs(filenames, loading_functions, processing_functions, outpath)

def get_missing_jobs(data_path: str, is_done: Callable, n_processes: int = 4) -> pd.DataFrame:
    path = Path(data_path)
    filenames = list(path.glob(f"**/*"))
    results = []
