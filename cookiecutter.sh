...
echo "Do you want to use a ${BOLD}preset${NORMAL}?"
echo "This will give you the best defaults (meaning you can skip everything after the command line interface choice)."
echo "Options are: ${BOLD}${RED}'student'${NC}${NORMAL}, ${BOLD}${RED}'research'${NC}${NORMAL} and ${BOLD}${RED}'package'${NC}${NORMAL}:"
read preset
...
echo "Do you work with conda and want to directly ${BOLD}${RED}install${NC}${NORMAL} the repo? (y/n)"
read conda
...
if [ "$conda" = "$yes" ] ; then
    ...
    if [ "$conda_new" = "$yes" ] ; then
        echo $(yellow "Please enter your project name")
        read name
        echo $(yellow "Please enter your python version")
        read version
        ...
        echo $(yellow "Is your conda env activated? If no, please enter the name of your env.")
        read env_name
